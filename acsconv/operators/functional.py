...
class ACSConvOp(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_output_a = grad_output[:, 0:acs_kernel_split[0]].contiguous()
        grad_output_c = grad_output[:, acs_kernel_split[0]:(acs_kernel_split[0]+acs_kernel_split[1])].contiguous()
        grad_output_s = grad_output[:, (acs_kernel_split[0]+acs_kernel_split[1]):].contiguous()
        ...
        if ctx.needs_input_grad[1]:
            ...
            if acs_kernel_split[0]>0:
                ...
                grad_weight_list.append(grad_weight_a.squeeze(2))
            if acs_kernel_split[1]>0:
                grad_weight_c = backward_cpp.backward_weight(
                    c_x, weight_c.shape, grad_output_c, stride, (padding[0],0,padding[2]), dilation, groups,
                    torch.backends.cudnn.benchmark, torch.backends.cudnn.deterministic
                ) 
                grad_weight_list.append(grad_weight_c.squeeze(3))
            if acs_kernel_split[2]>0:
                grad_weight_s = backward_cpp.backward_weight(
                    s_x, weight_s.shape, grad_output_s, stride, (padding[0],padding[1],0), dilation, groups,
                    torch.backends.cudnn.benchmark, torch.backends.cudnn.deterministic
                )    
                grad_weight_list.append(grad_weight_s.squeeze(4))
            grad_weight = torch.cat(grad_weight_list, dim=0).contiguous()
            
        if bias is not None and ctx.needs_input_grad[2]:
            grad_bias = backward_cpp.backward_bias(grad_output.contiguous())
